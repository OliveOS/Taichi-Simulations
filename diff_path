...
def diffuse_cell(i, j):
    ...
    for k in range(3):
        for l in range(3):
            if (i-1+k) >= 0 and (i-1+k) <= n and (j-1+l) >= 0 and (j-1+l) <= n: #if cell is in range add to sum
                sum += diffusion_map[i-1+k, j-1+l]
                logging.debug(diffusion_map[i-1+k, j-1+l])

    #get average and disperse
    avg: float = sum/9
    for k in range(3):
        for l in range(3):
            if (i-1+k) >= 0 and (i-1+k) <= n and (j-1+l) >= 0 and (j-1+l) <= n: #if cell is in range
                diffusion_map[i-1+k, j-1+k] = avg
...
gui = ti.GUI("Diffusion Path", res = (n, n))
